...
## HARD — knowledge-first section order

For each client MD, render schema narrative fields before inventories of files.
End every client MD with `## Evidence Mapping (Appendix)` from `evidence_appendix`.
Never put Completeness/gate reports in Risks_Gaps_and_Traceability.
