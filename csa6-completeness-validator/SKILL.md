---
name: csa6-completeness-validator
...
- Validate pack_substance 100% against that specialist's pack-schemas (and csa6-pack-schemas/output-schemas).
...
Lane/final report: `csa6-quality-gate-framework/schema.json`  
Pack gate: `csa6-gate-document/schema.json`

## HARD — load skills (do not restate)

- `csa6-parallel-lane-gates`
- `csa6-section-boundaries`
- `csa6-rich-content`
- `csa6-active-root-hygiene`
- Lane gates + **`csa6-gate-document`**
- `csa6-arc42-c4-views` / `csa6-mermaid-diagrams` (FINAL HTML + diagrams)
- Pack substance schemas under **`csa6-pack-schemas/output-schemas/`** (content contracts only — do not write `machine/` or invoke Assembler)
...
| **Final** | Render lean pack from accepted artifacts, then self-gate `csa6-gate-document` in the same turn. |
...
4. Validate each file **100%** against the matching `csa6-pack-schemas/output-schemas/*.schema.json` (`required[]`, `minItems`, ID patterns). If any schema fails → **FAIL** (`pack_output_schema_conformance`) with `schema_fields_missing` — **do not** write/overwrite client MD yet.
...
7. Validate:
   ...
   - **All required Mermaid diagram IDs present** (`csa6-mermaid-diagrams`) — blocking
   ...
   - `csa6-gate-document` + `csa6-rich-content` / `csa6-section-boundaries`
