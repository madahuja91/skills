---
name: stored-procedure-analysis-v25
description: >-
...
skill:
  ...
  inputs:
    ...
    - epic_names
  epic_names:
    when_empty: Extract all stored procedures from the uploaded codebase. No domain filter.
    when_provided: Extract only stored procedures that belong to the provided domain/capability names plus procedures those domains actually call.
  output:
  ...
  workspace_disk:
    ...
    - ../files/
    write_paths:
    - artifacts/packages/stored-procedure-evidence.json
    - artifacts/packages/stored-procedures/
    forbid:
    - /app/temp as write root
    - inventing absolute temp trees
    rule: Read SQL from workspace disk; write evidence only under <ACTIVE_ROOT>/artifacts/ (if cwd is src, that is artifacts/ — never src/artifacts/). Cite workspace-relative paths, never /app/temp as stored location.
  active_root:
    resolve:
    ...
    - <ACTIVE_ROOT>/artifacts/packages/stored-procedures/
    forbid:
    - mkdir new src when ACTIVE_ROOT exists
    - nested src/src
    - /app/temp as write root
    rule: Never create a new src directory. Write only under the resolved ACTIVE_ROOT artifacts tree. When cwd is src, use artifacts/... not src/artifacts/...
```
