...
Obey `csa5-specialist-worker` (do not restate).
...
3. Detect languages, frameworks, DBs, infra using descriptors first (`csa5-legacy-framework-heuristics`).
4. Scan for stored procedures / packages / ORM call sites (`csa5-legacy-stored-procedures`) and IBM MQ (`csa5-legacy-ibm-mq`) using heuristics only — never hardcode folder paths or package names.
5. Map internal/external dependencies with risk.
6. Emit `module_map` for Manager scoping; sync via `csa5-swarm-shared-memory`.
...
See `quality-rubric.md`. Gate: `csa5-gate-discover`.
