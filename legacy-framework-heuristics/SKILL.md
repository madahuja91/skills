...
| TopLink / EclipseLink `StoredProcedureCall` | ORM SP calls — see `legacy-stored-procedures` |
| iBatis/MyBatis `<procedure>` / `{call ...}` | SQL maps SP — see `legacy-stored-procedures` |
| `com.ibm.mq.*` / `mq.*` properties | IBM MQ — see `legacy-ibm-mq` |
...
3. Stored procedures count as business rules + lineage sources — load `legacy-stored-procedures`.
4. IBM MQ put/get is a first-class integration — load `legacy-ibm-mq`.
