---
name: csa5-legacy-framework-heuristics
...
| TopLink / EclipseLink `StoredProcedureCall` | ORM SP calls — see `csa5-legacy-stored-procedures` |
| iBatis/MyBatis `<procedure>` / `{call ...}` | SQL maps SP — see `csa5-legacy-stored-procedures` |
| `com.ibm.mq.*` / `mq.*` properties | IBM MQ — see `csa5-legacy-ibm-mq` |
...
3. Stored procedures count as business rules + lineage sources — load `csa5-legacy-stored-procedures`.
4. IBM MQ put/get is a first-class integration — load `csa5-legacy-ibm-mq`.
