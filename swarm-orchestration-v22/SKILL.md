...
- Logical dependency order is enforced through shared memory even when agents are
...
```yaml
governing_rule: Epic/Feature = WHAT. Technical Story = HOW. FAC/AAC = Feature-level.
  SAC = Story-level.
shape:
- Requirement REQ-###
- Epic EPIC-###
...
- Technical Story ST-### under Feature with area in [UI, BFF/API, Domain, Persistence,
  Messaging, Testing]
- SAC SAC-### under Story
inputs:
- codebase
