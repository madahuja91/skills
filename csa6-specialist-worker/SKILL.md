...
Policy details live in attached skills (`csa6-specialist-worker` and domain skills).

## HARD — obey these skills (do not restate them in prompts)

| Concern | Skill |
|---------|-------|
| Single ACTIVE_ROOT / disk paths | `csa6-active-root-hygiene` |
| Shared memory read/write / handoffs | `csa6-swarm-shared-memory` |
| Artifact envelope + IDs + evidence | `csa6-artifact-contract` |
| Evidence citation shape | `csa6-evidence-citation` |
...
Every specialist artifact MUST include `pack_substance` validating against that agent's `pack-schemas/` (copies of csa6-pack-schemas output-schemas). Missing sections/floors/IDs = incomplete.
