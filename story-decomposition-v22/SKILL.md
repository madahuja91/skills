...
- Testing is selected when dedicated technical validation is required; individual
  Stories must still include their own testing strategy.
- Identify dependencies and ordering between Stories.
- Detect whether a single Story would become too broad and split only by approved
  area.
- Write selected_areas and skipped_areas so the Technical Story manager can RUN selected
  agents and SKIP the rest.
